...
function scr_collision() {
	...
	// Horizontal
	var move_hsp = clamp(abs(hsp_final), 0, bbox_width) * sign(hsp_final); // Calculate max units to move.
	repeat ceil((abs(hsp_final) / bbox_width)) { // Repeat per Bbox Width.
	...
	// Vertical
	var move_vsp = clamp(abs(vsp_final), 0, bbox_height) * sign(vsp_final); // Calculate max units to move.
	repeat ceil((abs(vsp_final) / bbox_height)) { // Repeat per Bbox Height.
