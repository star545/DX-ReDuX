...
function scr_state_machroll() {
	if if_char(characters.jetpack_noise)
		scr_jetpack_machroll()
	else 
		scr_player_machroll()
	
}

function scr_jetpack_machroll() { 
	...
	scr_player_addslopemomentum(0.3, 0)
	mach2 = 100
	machslideAnim = true
	...
	if (!key_down or !grounded) && state != states.bump && !place_meeting_collision(x, y - 16) && !place_meeting_collision(x, y - 32) 
	{
		image_index = 0
		scr_soundeffect(sfx_rollgetup)
		state = states.mach3
