function scr_ratblock_destroy()
{
	with (obj_player)
	{
		...
		if (state == states.chainsaw || state == states.chainsawpogo || state == states.chainsawbump || state != states.mort && state != states.bombgrab && ((!scr_transformationcheck()) or state == states.barrel) && (place_meeting((x + 1), y, other) or place_meeting((x - 1), y, other) or place_meeting(x, (y + 1), other) or place_meeting(x, (y - 1), other)))
		{
			switch state
			{
				case states.cotton:
					if (place_meeting((x + 1), y, other) or place_meeting((x - 1), y, other)) && sprite_index == spr_cotton_drill || sprite_index == spr_cotton_attack {
						instance_destroy(other)
					}
				...
				case states.chainsaw:
					if global.cheat_pogoratkill && (place_meeting((x + 1), y, other) or place_meeting((x - 1), y, other)) {
						instance_destroy(other)
						break
					}
