function scr_player_rideweenie()
{
	...
	if (grounded && (!(place_meeting(x, (y + 1), obj_bounceplank))))
	{
		...
		scr_soundeffect(sfx_weeniebounce)
		vsp = -6
	...
	if key_jump
		input_buffer_jump = 0
	if place_meeting_solid(x + hsp, y) && !place_meeting_slope(x + hsp, y) && !place_meeting(x + hsp, y, obj_ratblock) && !place_meeting(x + hsp, y, obj_destructibles)
	{
		scr_soundeffect(sfx_weeniehitwall)
