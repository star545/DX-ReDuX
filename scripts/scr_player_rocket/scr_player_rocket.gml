function scr_player_rocket()
{
	...
	else
	{
		...
		if (floor(image_index) == (image_number - 1))
		{
			...
			image_index = 0
			movespeed = 16
			scr_soundeffect(sfx_rocketbegin)
		}
	}
	if key_jump
	{
	...
	if place_meeting_collision(x + sign(hsp), y, Exclude.SLOPES) && (!groundedSlope || place_meeting_collision(x + sign(hsp), y - 16, Exclude.SLOPES)) && !place_meeting(x + sign(hsp), y, obj_destructibles) && !place_meeting(x + sign(hsp), y, obj_metalblock) && !place_meeting(x + sign(hsp), y, obj_ratblock)
