...
if bbox_in_camera(view_camera[0], 0) && !seen {
	seen = true
	if do_help {
		scr_soundeffect_3d(sfx_toppinhelp)
	}
...
if (place_meeting(x, y, obj_player) && sprite_index == spr_pizzaboxunopen)
{
	...
	var snd = scr_soundeffect(sfx_collecttoppin)
	...
	if (content == obj_pizzakinshroom)
	{
		...
		if (global.toppintotal < 5)
			obj_badhud.message = (("YOU NEED " + string((5 - global.toppintotal))) + " MORE TOPPINS!")
		if (global.toppintotal == 5)
			obj_badhud.message = "YOU HAVE ALL TOPPINS!"
		obj_badhud.showtext = 1
		obj_badhud.alarm[0] = 150
	...
	with instance_create(x, y, obj_machalleffect) sprite_index = spr_pizzaboxopen
	instance_destroy()
}
