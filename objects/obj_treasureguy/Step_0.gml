...
if (sprite_index != scaredspr && state != states.underground && state != states.grabbed && state != states.hit && state != states.stun && state != states.chase && (!running))
{
...
{
	if use_heat() bombreset--
	if bombreset <= 0 && use_heat() {
		with instance_create(x, y, obj_junkhurt) {
