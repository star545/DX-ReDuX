...
with (obj_player)
{
	if (grounded && x > (other.x - 160) && x < (other.x + 160) && key_up && (state == states.normal or state == states.mach1 or state == states.mach2 or state == states.mach3) && (global.panic == true || (instance_exists(obj_arenamode_controller) && other.image_index = 1 && state != states.comingoutdoor) or room == war_13) )
	{
		var ex = x
		var ey = y
		...
		scr_savescore(global.leveltosave, instance_exists(obj_arenamode_controller))
