				...
				with (playerid)
				{
					...
					else if (!launched)
					{
						...
						if (state == states.mach3 or (state == states.machslide && sprite_index == spr_mach3boost))
							launch = 1
						if if_char(characters.jetpack_noise) && (state == states.mach3 or state == states.pogo || (state == states.Sjump && sprite_index == spr_sjumpcancel))	
							launch = 1
						if (state == states.machslide && sprite_index == spr_mach3boost && launch)
							movespeed -= 0.6
						freefallsmash = 0

						if (state == states.grind or state == states.climbwall or ((state == states.Sjumpprep or state == states.Sjump) && !if_char(characters.jetpack_noise)) or state == states.Sjumpland or y < (other.y - 400))
						{
