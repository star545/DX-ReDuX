/*with (obj_player1)
{
	if (character != "V")
	{
		if ((place_meeting((x + hsp), y, other) or place_meeting((x + xscale), y, other)) && (obj_player1.state == states.mach3 or obj_player1.state == states.rocket or (state == states.ratmount && ratmount_movespeed >= 12) || obj_player1.state == states.knightpepslopes or obj_player1.state == states.shoulderbash))
		{
			playerindex = 0
			instance_destroy(other)
		}
	}
	if ((state == states.ghost && ghost_pepper >= 4) || state = states.flick) && place_meeting(x + hsp, y + vsp, other)  {
		playerindex = 0
		instance_destroy(other)
...
{
	if (place_meeting(x, (y - 1), obj_player1) && (obj_player1.state == states.freefall || (obj_player1.state == states.ratmountgroundpound && obj_player1.brick)) && obj_player1.freefallsmash >= 10)
	{
