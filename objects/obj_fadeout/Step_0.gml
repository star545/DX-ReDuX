if (fadealpha > 1)
{
	...
	if instance_exists(obj_player)
	{
		if (custom_level == 0)
		{
			...
			{
				if music_to_normal { audio_sound_gain(global.music, (0.6 * global.option_music_volume), 0)
					audio_stop_all()
				}
				
				if room_exists(obj_player1.targetRoom)
...
if instance_exists(obj_player)
{
	with (obj_player1)
	{
		if (other.fadein == 1 && (obj_player1.state == states.door or obj_player1.state == states.victory) && (sprite_index == spr_victory or place_meeting(x, y, obj_door) or place_meeting(x, y, obj_startgate) or place_meeting(x, y, obj_arcademachinesmip)))
		{
	...
	if instance_exists(obj_player2)
	{
		with (obj_player2)
		{
			if (other.fadein == 1 && (obj_player2.state == states.door or obj_player2.state == states.victory) && (place_meeting(x, y, obj_door) or place_meeting(x, y, obj_startgate) or place_meeting(x, y, obj_arcademachinesmip)))
			{
				state = states.comingoutdoor
				image_index = 0
				if (global.coop == true)
