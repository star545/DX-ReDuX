...
if (state == states.walk)
{
	if (!chasing)
	{
		...
		if (targetplayer.x > (x - 150) && targetplayer.x < (x + 150) && y <= (targetplayer.y + 60) && y >= (targetplayer.y - 60))
		{
			chasing = 1
			state = states.charge
			//if object_index == obj_thug_green state = states.pizzagoblinthrow
...
else if (state == states.chase)
{
	...
	if inst_front != -4 if !place_meeting_solid(inst_front.x, inst_front.y) inst_front = -4
	...
	if inst_down != -4 if !place_meeting_solid(inst_down.x, inst_down.y) inst_down = -4
...
{
	bombreset = attackreset
	if (attack_count > 0) attack_count--
	else
	{
		if object_index != obj_thug_green {
