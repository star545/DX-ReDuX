if obj_player1.state == states.bossdefeat exit;
...
draw_text_scribble(480, _yy, "[fa_center]" + string(enemies_left) + " LEFT")
if instance_exists(obj_baddie) {
	...
	var dir = point_direction(_xx + camx, _yy + camy, inst.x, inst.y)
...
_hx -= (_tx / 2)
var _hy = 130
if global.option_heatmeter _hy += 40
if global.option_funnyhud _hy += 90
hp_index += 0.35
...
for (var i = 0; i < player_hp_max ; i++) {
	...
	var _hpy = _hy
	if i >= halfway { _hpy += 10 _xx += 10 }
	draw_sprite_ext(spr_bosshealth, hp_index, _xx, _hpy, 1, 1, 0, c_black, 1)
	if i < player_hp draw_sprite_ext(spr_bosshealth, hp_index, _xx, _hpy, 1, 1, 0, c_white, 1)
