if instance_exists(obj_keyconfig) || instance_exists(obj_audioconfig)
	exit;
scr_getinput()
var cur_page = page[selected_page]

/*
var total_option_length = array_length(options)
var suboption_move = (key_left2 + key_right2)
var option_move = (key_down2 - key_up2)
if option_move != 0 {
	array_selected += option_move
	scr_soundeffect(sfx_step)
}
array_selected = clamp(array_selected, 0, total_option_length - 1)

var cur_option = options[array_selected]
...
if !is_simple_function {
	total_suboption_length = array_length(input)	
	if suboption_move != 0 {
		options[array_selected].selected += suboption_move
		scr_soundeffect(sfx_step)
	}
	options[array_selected].selected = clamp(options[array_selected].selected, 0, total_suboption_length - 1)
}
