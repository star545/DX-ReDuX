...
if (sprite_index == spr_banditochicken_wake && floor(image_index) == (image_number - 1))
{
	...
	state = states.charge
	if use_heat() state = states.chase
	movespeed = 8
	with (instance_create(x, y, obj_jumpdust))
		image_xscale = other.image_xscale
}
if (state == states.charge && use_heat()) state = states.chase
var a = state == states.charge || state == states.chase
if (a && bonebuffer > 0)
	bonebuffer--
if (grounded && jumping < 40 && a)
	jumping++
if (a && grounded && jumping >= 40)
{
	vsp = -11
	jumping = 0
